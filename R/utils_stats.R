count_unique <- function(x, na.rm = TRUE) { length(unique(x)) }
...
froese_binohlan <-  function(lmax, lengthData){
  ...
  selectivity = dplyr::case_when(p_obj < 1 & percentOpt / 100 + percentMega / 100 == 0 & percentMature / 100 > 0.25 ~ "Fish small and immature",
  ...
  status = dplyr::case_when(p_obj < 1 & percentOpt / 100 + percentMega / 100 == 0 & percentMature / 100 > 0.25 ~ "Spawning biomass above reference point",
